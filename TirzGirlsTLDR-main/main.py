...
async def handle_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if f"@{bot_username}" in text.lower():
        ...
        if not prompt:
            await msg.reply_text("👀 I’m here — give me something to work with, boo.")
            return
        try:
            completion = client.chat.completions.create(
                model="gpt-4",
                messages=[
                    {"role": "system", "content": "You are Summaria, a flirty, shady, and helpful assistant. Talk like you're in the group chat — a little glam, a little mean, but always helpful."},
                    {"role": "user", "content": f"Reply directly to this without saying my name: {prompt}"}
                ]
            )
            reply = completion.choices[0].message.content.strip()
        except Exception as e:
            reply = "Ugh. I tried, but something went wrong with my brain. Try again later 💅🏾"
