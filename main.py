def main():
    ...
    app.add_handler(CommandHandler("notifyrestart", notify_restart))  # New manual notify command
    app.add_handler(CommandHandler("memories", memories_cmd))  # New memories command
    app.add_handler(CommandHandler("forget", forget_cmd))  # New forget command (owner only)
...
async def tldr(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        if "rate_limit" in str(e).lower():
            reply = "OpenAI is being slow right now, try again in a few minutes babe 🐌"
        else:
            reply = "My brain glitched trying to summarize that 😵‍💫"
